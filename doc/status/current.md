...
- **已验证（BL31 基础设施）**：从当前 R1 `p1` FIT 提取的 image 1 是 194,076 B 的实际 ARM Trusted Firmware 载荷，SHA-256 `045b2cef2942b527ef832a6126233d4ac2ded6ba6c26d40bff6a03c96a1eab1d` 与 FIT 声明一致。其字符串含 `rockchip_sip_svc`、`rockchip_plat_sip_handler` 与未知 SiP 报错路径，证明 BL31 有 Rockchip SiP 分发基础；没有直接得到 `RK_SIP_AMP_CFG (0x82000022)` 的支持证据，不能调用 SMC 试探。见[EXP-20260820-002](../experiment/exp-20260820-002-inspect-r1-bl31-sip-entry.md)。
...
在主机已取得的源码与资料中定位实际 BL31 对 `RK_SIP_AMP_CFG (0x82000022)` 的处理证据：寻找匹配的 Rockchip SiP handler 或分发表；若本地缺失，记录缺口后再决定是否获取匹配 BSP/TF-A 源码。暂不向板端发出 SMC、不加载 AMP DTB、不启动第二个 CPU、不写 eMMC。
