...
- **MailMsg 协议记录**：专门协议笔记记录双向×4 priority、每 ring 8 个物理槽/可用上限 7、SPSC、cache/barrier/commit 语义及 mailbox doorbell 角色；V1 的 48 B frame 为历史基线，当前 V2 固定 frame 为 52 B。见[MailMsg 协议笔记](../note/mailmsg-protocol.md)。
- **MailMsg 协议 V2 状态**：ABI 已升级为 V2，`version=2` 并新增 `crc32`。修正 DTS sysfs loader size `0x9030→0xa030` 后，V2 RAM-only candidate 已完成一次 mailbox0 ch0 正常路径：Zephyr `image=41008/41008`、`CPU_ON ret=0`，priority 0 value 41 返回 `type=2 seq=1 value=42`，`m0c0 rx 0xb2a10000/0x0, tx 1/0`、pending 0。通用 host C unit test 的 payload corruption → reject 仍是篡改拒收唯一证据；本次不证明板端篡改拒收或故障恢复。`commit`/`ready` 仍是必要发布标记，不是 CRC 替代。见[MailMsg 通信协议设计](../note/mailmsg-protocol.md)与[EXP-20260822-001](../experiment/exp-20260822-001-build-r1-amp-shmem-ping.md)。
...
唯一下一步：在不改变启动分区的前提下，验证 V2 板端 payload 篡改拒收及最小错误边界；随后再验证队列满策略与异常/重试边界。当前仅有 CRC32 正常路径板端证据和 host corruption → reject 证据，暂无板端故障恢复证据；暂不将 RPMsg 作为主路径，也不引入并发、高负载、大数据、外设、mailbox1/2 写入或长期 LLM 负载。
