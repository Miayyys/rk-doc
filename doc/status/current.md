...
- **已验证（BL31 基础设施）**：从当前 R1 `p1` FIT 提取的 image 1 是 194,076 B 的实际 ARM Trusted Firmware 载荷，SHA-256 `045b2cef2942b527ef832a6126233d4ac2ded6ba6c26d40bff6a03c96a1eab1d` 与 FIT 声明一致。其字符串含 `rockchip_sip_svc`、`rockchip_plat_sip_handler` 与未知 SiP 报错路径，证明 BL31 有 Rockchip SiP 分发基础；没有直接得到 `RK_SIP_AMP_CFG (0x82000022)` 的支持证据。当前本机 `src/` 与 `R1/` 也未找到可定位的匹配 TF-A/BL31 源码或 handler 符号，故不能调用 SMC 试探。见[EXP-20260820-002](../experiment/exp-20260820-002-inspect-r1-bl31-sip-entry.md)。
...
只读列出已有 `src/rkbin` 的 BL31 二进制并与实际 `r1-atf-1.bin` 做 SHA-256 比对：若有精确相同载荷，取得其文件名与 Git 来源线索；没有匹配也记录为证据。暂不向板端发出 SMC、不加载 AMP DTB、不启动第二个 CPU、不写 eMMC。
