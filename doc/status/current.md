...
- **MailMsg 协议记录**：专门协议笔记记录双向×4 priority、每 ring 8 个物理槽/可用上限 7、SPSC、cache/barrier/commit 语义及 mailbox doorbell 角色；V1 的 48 B frame 与 V2 的 52 B frame 为历史验证 ABI，当前候选共享区头 `version=3`，即 V3 draft。见[MailMsg 协议笔记](../note/mailmsg-protocol.md)。
...
唯一下一步：在不改变启动分区的前提下，验证 MailMsg V3 ACK/NACK 与上层策略在并发、压力和异常边界下的行为；自动重传仍不由协议实现。暂不写入 eMMC、将 RAM-only 证据扩展为持久化结论，也不将 RPMsg 作为主路径或引入长期 LLM 负载。
