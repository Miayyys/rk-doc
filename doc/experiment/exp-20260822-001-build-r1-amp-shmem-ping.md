...
### 步骤 38：RAM-only MailMsg V3 用户态字符设备回归

目的与预期结果：在不改变持久化启动介质的前提下，首次验证用户态 priority-scoped 字符设备的 `write`、`poll`、`read` 代表路径，并避免旧 sysfs 接口与字符设备同时消费同一响应队列。

本次事件时间为 2026-08-29。RAM-only FIT 为 `build/local/r1-mailmsg-endpoint/r1-mailmsg-userdev-external.img`，大小 `38636544 B`，SHA-256 为 `4daeab6d92b2450b4e31e992ec7607b0e6890e9d134b225901eb5016234c87cf`。候选启动到 Linux `5.10.252`、`nproc=7`；加载 Zephyr `mailmsg-userdev.bin` 并执行 PSCI start 后，板端出现 `/dev/mailmsg-p0`、`/dev/mailmsg-p1`、`/dev/mailmsg-p2`、`/dev/mailmsg-p3`。本次未读取旧 sysfs `mailmsg_response`，以避免双消费者。

使用静态 AArch64 client 经字符设备 p0 写入 PING `41`，再通过 `poll/read` 得到：ACK `priority=0 type=3 sequence=1 length=8 peer_sequence=1 status=0`，随后 PONG `priority=0 type=2 sequence=2 length=4 value=42`。经 p2 写入 PING `100`，通过 `poll/read` 得到 PONG `priority=2 type=2 sequence=3 length=4 value=101`，符合 p2 无 ACK/NACK 的策略。
...
### 步骤 56：R6 里程碑冻结与长期证据附件（本机冻结完成）

目的与预期结果：保存 R6 已确认源码与五份脱敏板端报告的可追溯关系，形成不提交大型产物的本机冻结目录；本步骤不改变源码、板端状态或 Git 历史。

本次事件时间为 2026-08-31。主仓库已本地 commit `cdeb93dc3361fd30047cef9d88cd6e35297fd4d1`（`mailmsg: complete R6 AMP lifecycle milestone`），用户授权后以精确 `force-with-lease` 将远端 GitHub `origin/main` 从 `d332d020d346cc3d87427cc1409c8d783a0e2f47` 更新到 `6fdcff9bcfa41113d5f1384332fc34f91a488caf`；本地主分支已由 `master` 重命名为 `main` 并跟踪 `origin/main`，旧本地 main 保留为 `main-pre-r6-backup`。内核仓库 `study/r1-dts-port` 仍有本地 commit `dcb7dfd7b5f734c8d817920129ca4b794d219c9b`（`soc: rockchip: add MailMsg R6 lifecycle control`）；远端仍为 Rockchip `origin`，没有个人 upstream，内核 commit 尚未推送，但其 format-patch 已进入只读冻结目录。冻结目录为 `build/local/releases/mailmsg-v1-r6-20260831/`，大小约 77M，包含最终 FIT、Image、DTB、resource、四个 Zephyr bin、三个板端工具、ITS、五份报告及该内核 commit 的 format-patch；`final freeze-manifest.md` 与 `SHA256SUMS` 已生成，SHA-256 分别为 `4ca2b1a7b4a3130c06e817124ab418d246e797667a5562dbc399e22a9e8e13b2` 与 `ba240f61a1011503adebdd2c9c86c3c8c2fe66c64a52c7850da2b9be62630fef`。`sha256sum -c` 对 18 个 artifact/report/patch 全部通过，随后目录已执行 `chmod -R a-w`。冻结包现为本机冻结完成，不将大型产物提交。
